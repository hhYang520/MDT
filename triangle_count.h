      ...
      if (u < v) {
        ...
        for (auto &u3_ifo: G.nodes[u].TE) {
          ...
          if (G.nodes[v].TE.find(w) != G.nodes[v].TE.end()) {
            if (static_triangle[pair<int, int>(u1, v1)].find(w1)
                == static_triangle[pair<int, int>(u1, v1)].end()) {
              ...
              max_interval = max(find_max_interval(G.nodes[u1].TE[v1].TS, G.nodes[u1].TE[w1].TS, G.nodes[v1].TE[w1].TS),
                                 max_interval);
            }
          }
        }
